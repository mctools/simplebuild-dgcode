"""Module for creating scan jobs to be launched either at dmsc or locally"""
...
class ScanLauncher:
    ...
    def _create_rundir_and_script(self,rundir,cmd,setupsh):
        os.mkdir(rundir)
        from Utils.runscript import create_run_script
        create_run_script(os.path.join(rundir,'run.sh'),cmd=cmd, setupsh = setupsh,
    ...
    def go(self):
        ...
        if opt.launch:
            ...
            for i,(label,cmd) in enumerate(self.job_cmds()):
                ...
                if opt.queue=='local':
                    self._create_rundir_and_script(rundir,cmd,setupsh)
