...
def cmphists(histograms_in,
             ...
             normalise='%',
             **kwargs):
    """Overlay plot of passed in histograms. histograms_in is a list [(legendkey,histogram)]. Rebin and normalise arguments are used to control rebinning and normalisation. All other arguments are passed along to SimpleHists.Hist1D.overlay"""
    global __ihist
    global __html
    __ihist+=1
    ...
    def modhist(h):
        if (rebin!=None and rebin!=h.nbins) or normalise:
            h=h.clone()
            if (rebin!=None and rebin!=h.nbins):
                h.rebin(rebin)
            if normalise:
                h.norm()
                if normalise=='%':
