...
namespace G4NCrystalRel {
  ...
  public:
    ...
    G4Material * getFinalMaterial( const NC::MatCfg& cfg ) {
      ...
      if (s_verbose) {
        auto mgr = ::G4NCrystalRel::Manager::getInstance();
        ...
        G4cout<<"G4NCrystalRel::The material: ---------------------------------------------------------------------"<<G4endl<<G4endl;
        G4cout<<" Material index in table: "<<mat->GetIndex()<<G4endl;
        G4cout<<*mat<<G4endl;
        G4cout<<"G4NCrystalRel::The base material: ----------------------------------------------------------------"<<G4endl<<G4endl;
    ...
    G4Material * getFinalMaterialImpl( const NC::MatCfg& cfg ) {
      ...
      G4NCrystalRel::Manager::getInstance()->addScatterProperty(mat,std::move(scatter));
...
G4Material * G4NCrystalRel::createMaterial( const char * cfgstr )
{
  try {
    ...
    Manager::handleError("G4NCrystalRel::createMaterial",101,e);
  }
  return 0;
}

G4Material * G4NCrystalRel::createMaterial( const G4String& cfgstr )
{
  //Just delegate to const char * version:
  return createMaterial(cfgstr.c_str());
}

G4Material * G4NCrystalRel::createMaterial( const NC::MatCfg&  cfg )
{
  try {
    auto& db = objDB();
    NCRYSTAL_LOCK_GUARD(db.mtx);
